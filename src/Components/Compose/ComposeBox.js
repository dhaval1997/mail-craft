...
import { saveDraft, sendMail, deleteMails } from "../../Store/MailAction";
...
const ComposeBox = () => {
  ...
  const editorContent = useSelector((state) => state.general.editorContent);
  const fireBase = useSelector((state) => state.mail.fireBase);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      sendMail({
        ...
        receiverEmail: mailInput.current.value.replace(/[^a-z0-9A-Z]/gi, ""),
  ...
  return (
    <div
      ...
      onClick={(e) => {
        e.target.classList.contains("modalClose") && dispatch(closeCompose());
      }}
    >
      <div className="sm:w-96 w-72">
        <div className="bg-slate-900 px-1 py-1.5 rounded-t-md flex items-center justify-center relative">
          <div className="absolute right-0 flex items-center space-x-2.5 pe-1">
            ...
            <button onClick={() => dispatch(closeCompose())}>
              <svg
            ...
            <button onClick={() => dispatch(deletingCompose())}>
              <svg
                viewBox="0 0 28 28"
                className=" w-4 h-4 stroke-slate-200 fill-slate-200 hover:stroke-white hover:fill-white"
                xmlns="http://www.w3.org/2000/svg"
